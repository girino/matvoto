...
public class BadTwoRoundVote implements VoteSystem {
	...
	public int getWinner(Voter[] voters, int[] counts, int[] candidates) {
		...
		for (int i = 0; i < counts.length; i++) {
			int count = counts[i];
			Voter v = voters[i];
			int prefferedCandidate = v.badPreferredCandidate(candidates);
		...
		for (int i = 0; i < counts.length; i++) {
			int count = counts[i];
			Voter v = voters[i];
			int prefferedCandidate = v.badPreferredCandidate(secondRound);
